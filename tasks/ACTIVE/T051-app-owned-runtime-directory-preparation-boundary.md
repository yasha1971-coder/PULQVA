...
- executable destinations are isolated under `runtime/bin/*` and that directory is not created by this task;
- no Arti, yt-dlp, or FFmpeg executable is created, copied, downloaded, or modified;
- existing symlink/non-directory hazards inside the runtime root fail closed before creation;
- every prepared directory canonicalizes beneath the verified runtime root;
...
- Windows and Linux desktop tests execute and remain green;
